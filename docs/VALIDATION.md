...
Settings → Shortcuts records key combinations through a local AppKit monitor before menu equivalents or text editors can handle them. Typer suspends its own registrations during recording, waits for release before saving/reactivating, and cancels on Escape, outside clicks, focus loss, or view removal. Persisted physical key codes use normalized Command/Control/Option/Shift modifiers; Caps Lock, Fn and keypad state do not distinguish bindings. Bare function keys and modifier-only release chords are supported. Modifier-only monitoring requires Accessibility, ignores Typer's own synthesized events, and cancels a chord when another key is used.

Carbon registration consumes key shortcuts in other apps and suppresses repeated action callbacks while a key is held. Rebinding probes registration before saving; conflicts preserve the old choice. Arming checks actual Pause/Skip availability and requires at least one Stop binding. The configured Stop and reserved Control–Esc backup are reported separately. In-app previews route controls to their own session rather than starting external playback.
...
2. Choose the playback mode, click **Run Typer**, and leave the editor focused. **Fixed delivery check** uses steady timing. For **Everyday text**, **Natural rhythm** offers seeds 1, 2 and 3 with the production generator, baseline profile and default settings (64 WPM, variation 0.78, mistake level 2). Test all three seeds; do not select the closest run. Your reference is never used to train these fixtures. Each native result includes its generator, seed, settings, repair count and planned event count. This does not evaluate a learned profile. The helper uses the production scheduler, event factory and process-addressed delivery. It checks foreground Safari, the current U.S. layout, and the uniquely labelled test editor before each key-down. A separate page instance cannot satisfy that editor identity. Focus loss, stale heartbeat or Stop aborts playback and releases posted keys. These extra focus checks add measurement overhead, especially before the first press; the receipt timings are not a hardware-latency benchmark.
3. Click **Compare samples**. The report checks final text, key balance, event types/order, key/code/location, modifiers, input/composition data and `isTrusted`. It separately compares property sets for each shared logical key, keeping physical code and location in the comparison. Counts and groups present on only one side remain visible in the detailed report. A shared-key match never overrides a failed or different full sequence. Corrections and overlap can legitimately change full event order. Selection changes are retained but excluded from exact equality because browsers can coalesce them. Input events without a preceding `beforeinput` invalidate the capture.
...
The fixed passages cover plain text, capitals/symbols, overlap, backspace/Option-backspace, line breaks, Unicode and every direct Option symbol in the installed U.S. mapping. `UCKeyTranslate` must resolve each direct mapping without leaving a dead-key state; the ordinary ASCII combinations are preferred where available. The page disables spellcheck/autocorrection. Tab is deliberately excluded: ordinary web textareas normally move focus on Tab, whereas the AppKit diagnostic inserts a tab. IMEs, rich-text editors, shortcuts, substitutions and other sites still need their own checks.
...
`node scripts/browser-check/run-safari.mjs SESSION_URL OWNED_SAFARI_WINDOW_ID OUTPUT_FOLDER` runs every fixed fixture plus all three Natural seeds in one already-owned test window. It navigates only while that window still contains the session URL and stops the suite on an unusable capture. It uses ordinary Safari navigation, not WebDriver or Apple-event JavaScript. This runner temporarily needs Safari focus; it never creates a physical reference. Generated captures are saved as `plain-natural1.json`, `plain-natural2.json`, and `plain-natural3.json`; compare each against the same separately recorded plain reference. Every native run can also be triggered individually from the page.
