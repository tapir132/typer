...
- A physical reference was subsequently supplied. Its plain-key properties matched for the shared keys, while its corrections and rollover changed the full sequence. The original fixed fixture did not exercise the rhythm generator, so this could not validate the generated cadence.

## Follow-up: direct mappings and generated rhythm

1. Read Apple's installed U.S. layout with `UCKeyTranslate` to cover the remaining direct Option/Shift–Option characters. Require a printable complete character and zero dead-key state. Preserve the Unicode fallback for characters that need composition; do not fabricate hardware provenance.
2. Add native Safari runs using `TypingEngine.generatePlan` with the default Natural settings and preselected seeds 1–3. Keep the supplied physical reference out of training and parameter selection. Record run metadata and report every seed.
3. Separate shared logical-key properties from positional event-sequence comparisons. Retain counts, one-sided keys, full sequence differences, timing distributions and overlap. Add regressions proving that grouping cannot hide incorrect key codes, modifiers, locations or extra events.
4. Rerun the complete Safari suite and compare all three generated runs with the supplied reference. These observations can reveal concrete defects; one short human recording cannot certify a personal model or universal indistinguishability.

Research basis: [Apple's layout translation API](https://developer.apple.com/documentation/coreservices/1390584-uckeytranslate), [UI Events keyboard/composition properties](https://www.w3.org/TR/uievents/), and [Aalto's observations of rollover and individual typing variation](https://userinterfaces.aalto.fi/136Mkeystrokes/). Native event-source metadata remains part of the platform's observable input path.
