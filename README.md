...
For an external-browser check, run **`./scripts/verify-safari.sh`** from the repository. The local Safari page can record an explicit physical-keyboard sample, run fixed delivery checks or three seeded Natural-mode passages, and compare/export the received events. Reports separate shared-key properties from full event order and timing. It needs Node.js, the U.S. keyboard layout and Typer's existing signing identity/Accessibility grant. It does not require Safari Remote Automation or Apple-event JavaScript. [Instructions and interpretation](docs/VALIDATION.md#safari-input-comparison).

On the U.S. layout, Typer reads Apple's direct Option and Shift–Option combinations, including punctuation, mathematical symbols and directly available accented letters. Mappings must produce a complete character without a pending dead key. Accents requiring composition and emoji retain the Unicode fallback; their browser event sequences can differ from a physical input method. Playback still expects U.S. QWERTY.
...
- **Timing variation:** lower gives steadier pacing; higher adds bigger timing changes, quick bursts, and brief hesitations. Even 0% retains some natural or learned variation. The percentage is a setting, not a human score. Typing speed, mistake frequency, and long pauses have separate controls.
