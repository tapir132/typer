...
final class TypingScreenOverlay {
    ...
    func update(state: RunState, progress: PlaybackProgress, target: String, message: String?, enabled: Bool, shortcuts: ShortcutBindings = ShortcutBindings(), stopText: String? = nil) {
...
private struct TypingOverlayHost: View {
    ...
    var body: some View {
        TypingOverlayContent(paused: presentation.paused, progress: presentation.progress,
                             target: presentation.target, message: presentation.message, shortcuts: presentation.shortcuts, stopText: presentation.stopText)
    }
}
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 22) {
                ...
                Text(shortcuts.pause.displayText).font(.system(size: 72, weight: .bold, design: .monospaced))
                ...
                HStack(spacing: 36) {
                    Text("\(stopText ?? shortcuts.stop.displayText)   Stop")
                    Text("\(shortcuts.skipWait.displayText)   Skip long wait")
