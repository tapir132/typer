...
struct AppGuideView: View {
        ...
        VStack(alignment: .leading, spacing: 26) {
            ...
            HStack(alignment: .top, spacing: 32) {
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("⌘⌥P  Pause / resume\n⌘⌥→  Skip long wait")
                        .font(.system(size: 11, weight: .medium, design: .monospaced)).lineSpacing(5).padding(.top, 6)
    ...
    @ViewBuilder private var topicContent: some View {
        ...
        case .firstRun:
            ...
            section("Try Play preview first", "Click Play preview above Source text, then Play preview in the window that opens. Watch your actual text appear, including corrections and labeled waits. This stays inside Typer and needs no Accessibility permission. Close it when ready; Arm typing uses that same plan if you have not changed the source or settings.")
            ...
            section("5. Let the run finish", "A red fullscreen overlay shows the pause shortcut, destination app, progress, and any long wait. Keep the destination text field focused. The overlay passes mouse clicks through and does not take keyboard focus. Generated mistakes have planned corrections; the editor's autocorrect, shortcuts, or formatting can still affect the result, so check the finished text.")
            section("Pause, resume, or skip a wait", "Press ⌘⌥P to pause. Typer releases held keys and freezes the run's clock. Focus the same field and press ⌘⌥P again to continue. Switching apps also pauses the run; resuming requires the original app to be active. Avoid editing or moving the cursor in the unfinished text, especially during a correction. ⌘⌥→ skips only the remaining long wait and keeps later typing at its planned pace.")
        ...
        case .controls:
            section("Remember settings and save presets", "Compose remembers your controls across launches. Choose Quick messages, Long-form writing, or Clean copy from Preset, or adjust the controls and click Save… to name your own setup. Choose a saved preset to restore it; Delete saved preset removes a personal setup. You can keep 30 personal presets. Presets include the overlay and pause controls, but do not save source text or select a different learned profile.")
            ...
            section("Sentence pauses", "Turn on Sentence pauses in Compose for a pause after every detected sentence that has more text after it. The default range is 2–10 seconds, sampled separately at each ending. Adjust Min and Max from 1–60 seconds; equal values give a fixed wait. This works in every mode and does not require Thought pauses. If an existing pause is longer, Typer keeps that wait instead of adding them together. Closing quotes and punctuation clusters stay together; unusual abbreviations or formatting can still confuse sentence detection. There is no extra wait after the final sentence. The estimate includes the pauses, and the usual Stop shortcuts interrupt them.")
            section("Learned pause habits", "In My rhythm, this uses new recordings to estimate how often you pause within words, between words, and at sentence boundaries. Each category needs at least 20 opportunities, three observed pauses with retained durations, and two sessions before it influences generation. Rates and durations remain blended with fallback behavior. When sentence evidence is available, it replaces the occasional generic thought stall; the separate Sentence pauses control still supplies your chosen minimum wait. Copy and Freewrite habits stay separate. Profiles → Training coverage shows what is available.")
            section("Fullscreen typing overlay", "This is on by default. Turn Fullscreen typing overlay off in Compose to hide the red screen tint and large shortcut reminder. The shortcuts still work. The overlay covers connected screens during playback and remains visible with a lighter tint while paused. Stop or completion removes it. Mouse clicks pass through, so use the displayed keyboard shortcuts to pause or stop.")
            section("Reading and playing the preview", "The waveform is a compact illustration of timing, with highlighted backspaces. Play preview opens an in-memory editor and runs the exact plan on Typer's playback clock. It labels sentence, learned, thought and repair waits, with pause/resume, skip wait and stop controls. Skipping changes only that preview run. No keys go to another app, and no preview text is saved. External editors can interpret corrections differently. Once refreshed, the preview plan is also the plan used by Arm typing; changing text, settings, or the active profile refreshes it.")
        ...
        case .training:
            ...
            section("Check what the profile has learned", "Open Profiles → Training coverage for the active profile's timing, rollover, correction and pause counts. Available means an engineering support threshold is met; it is not a realism grade. Collect more means that feature still has limited evidence. Pause habits need repeated sessions, and some people rarely pause inside words. Type normally rather than inserting pauses to fill the checklist.")
            note("Keep your existing current profile", "Profiles with paired timing evidence remain useful and do not need clearing. Older current samples may lack the new pause summaries, so Typer uses fallback pauses until fresh recordings provide enough evidence. Legacy profiles remain labeled and locked as before.")
        ...
        case .validation:
            section("1. Collect comparable sessions", "Save at least four current sessions in the same training mode, each with at least 20 usable paired timings. Finish Copy or Sprint passages exactly to verify matched text. Partially completed passages can still train the profile, but their comparisons are labeled unmatched. Older current samples stay usable; if passage completion was not recorded, it is shown as unverified. Legacy samples without paired timings cannot be validated.")
            section("2. Open the report", "Go to Profiles → Validate rhythm. Choose a training context to see its ready-session count; Copy, Sprint, Freewrite and Live are never mixed. Typer keeps the two latest eligible sessions out of training and fits a temporary comparison profile using up to five earlier sessions. This does not replace your active profile. The comparisons run locally.")
            section("3. Start with the overview", "Overview shows Natural and My rhythm side by side: median W1 distance across paired runs, with the minimum–maximum range below. Each run uses the same held-out session, target WPM and seed for both modes. The three seeds (17, 41 and 89) reuse two human sessions, so they are not six independent human tests. Human vs human supplies context, not a pass threshold; the two sessions can contain different passages. Detailed trials shows each run and its observation counts.")
            section("4. Read distances in context", "W1 distance measures the separation between distributions in the feature's units, such as milliseconds for key hold. KS distance measures their largest cumulative difference on a 0–1 scale. Lower means closer for that feature. Check the retained counts, missing holds, rollover, and the human-session-versus-human-session comparison before drawing conclusions. Detailed trials also shows pause frequency, observed pauses and eligible opportunities by context. A close pause duration does not mean the model pauses at the right frequency; a dash means unavailable, while zero means eligible opportunities with no observed pauses.")
            ...
            section("6. Check actual playback separately", "Open Profiles → Check playback on this Mac. Choose Rhythm & overlap, Corrections, or Punctuation & Unicode, then click Run check. Keep the test window focused and do not type. The built-in text goes only to that window. Stop check or Escape interrupts it. A change of focus also stops the check.")
            section("Reading a playback check", "The report checks exact text, event counts, order and modifier flags. Timing is separate: median and p95 absolute errors show how closely key holds, press intervals and signed flights reach the editor. Arrival vs event timestamp reveals time spent waiting in a queue. A text/event pass is not a timing certification. The check uses Typer's scheduler and an AppKit editor, but other apps and input methods can behave differently. Export report saves only the built-in test and its measurements.")
        ...
        case .privacy:
            ...
            section("Settings, presets, and preview text", "Compose controls and named presets are saved locally. Compose source text and the animated preview stay in memory and are not restored after quitting. Learned pause summaries contain counts and bounded durations by category; they do not add an ordered text recording. Deleting learned data leaves your controls and presets intact; personal presets can be removed from the Preset menu.")
        ...
        case .updates:
            ...
            section("Why a local build can be newer", "Updates are ordered by the internal build number. Clean local builds use the commit timestamp, matching Edge for that commit, so rebuilding an older checkout still allows newer Edge updates. Builds with uncommitted app changes use the current time to protect those changes from an older published build. The displayed local build date records when the app was built.")
        ...
        case .troubleshooting:
            ...
            section("The text lands in the wrong place", "Typer remembers the destination app and pauses when another app becomes active, but it cannot lock or restore the exact insertion point. It cannot detect every field change within the same app. Stop with ⌘ Esc or ⌃ Esc if the destination is wrong, then decide whether to remove partial output before trying again.")
            section("Pause will not resume", "Activate the original destination app and click the same text field, then press ⌘⌥P. Typer will not resume into a different app. If the document or insertion point has changed, stop and start a new run after checking the partial text. A shortcut conflict at startup is shown as an error; release the conflicting ⌘⌥P or ⌘⌥→ binding in the other app before retrying.")
