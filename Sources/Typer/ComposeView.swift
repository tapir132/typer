    ...
    private var controlRail: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(spacing: 5) {
                ...
                compactToggle("Learned pause habits", note: profiles.activeProfile.trainingMode == .liveCapture ? "Use Freewrite to learn thinking pauses" : "My rhythm · uses recorded boundaries", help: "Uses your recorded pauses when enough training is available. Requires My rhythm. See Profiles → Training coverage for readiness.", isEnabled: model.isUsingLearnedProfile && !profiles.activeProfile.isLegacy && profiles.activeProfile.trainingMode != .liveCapture, binding: $model.settings.learnedPauses)
